<template>
  <div class="relative mt-4 mb-8">
    ...
    <div
      ...
      :class="
        open
          ? 'border-[#005F39] rounded-t-[5px]'
          : 'border-[#c1c1c1] rounded-[5px]'
      "
    >
      <span class="leading-[40px] w-[calc(100%-20px)] text-left block truncate text-gray-600 cursor-pointer
      select-none
      " 
      ref="displaySelect"
      >{{ displayValue }}</span>
    ...
    >
      ...
      :key="index">
        <div v-if="mode=='multiple'" class="border-r h-full w-[35px] select-none flex items-center justify-center">
...
const displayValue = computed(() => {
  if (props.mode === 'single') {
    ...
    return props.options.filter((option) => model.value.includes(option.id)).map((option) => option.text).join(', ');
  }
});
</script>
